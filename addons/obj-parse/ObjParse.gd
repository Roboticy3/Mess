	...
	for line in lines:
		...
		match parts[0]:
			...
			"newmtl":
				...
				if debug: print("Adding new material " + parts[1])
			...
			"Kd":
				# Diffuse color
				currentMat.albedo_color = Color(float(parts[1]), float(parts[2]), float(parts[3]))
				if debug: print("Setting material color " + str(currentMat.albedo_color))
				pass
			"map_Kd":
				# Texture file
				currentMat.albedo_texture = _get_texture(path, parts[1], debug)
			"map_Ks":
				# Texture file
				currentMat.albedo_texture = _get_texture(path, parts[1], debug)
			"map_Ka":
...
static func _get_texture(mtl_filepath, tex_filename, var debug:bool = false):
	if debug: print("    Debug: Mapping texture file " + tex_filename)
	var texfilepath = mtl_filepath.get_base_dir() + "/" + tex_filename
	var filetype = texfilepath.get_extension()
	if debug: print("    Debug: texture file path: " + texfilepath + " of type " + filetype)
...
static func parse_obj(obj_path, mtl_path, var debug:bool = false) -> Mesh:
	...
	for matgroup in faces.keys():
		if debug: print("Creating surface for matgroup " + matgroup + " with " + str(faces[matgroup].size()) + " faces")
