...
func call_option(p:Piece, o) -> bool:
	...
	var p_o := p.options
	if p_o.has(o):
		if p_o[o] is Callable:
			call_option_direct(p_o[o])
		elif p_o[o] is Dictionary:
			add_state(p_o[o])
			build_options()
		return true
...
func call_option_virtual(o:Callable, cur:=current_state.duplicate()) -> Dictionary:
...
func build_options():
	...
	Accessor.a_print(
		...
		"Start!\n" + Accessor.shaped_2i_state_to_string(cur, shape)
	)
	...
	for pos in cur:
		...
		for o in p_o:
			p_o[o] = call_option_virtual(p_o[o], cur)
			pass
